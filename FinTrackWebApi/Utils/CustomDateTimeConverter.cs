    ...
    public class CustomDateTimeConverter : JsonConverter<DateTime>
    {
        ...
        public override DateTime Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            if (reader.TokenType == JsonTokenType.String)
            {
                ...
                if (DateTime.TryParseExact(dateString, ExpectedApiFormat, CultureInfo.InvariantCulture,
                ...
                // ---- /ÖNEMLİ DEĞİŞİKLİK BURADA ----
                {
                    ...
                    if (result.Kind != DateTimeKind.Utc)
                    {
                        ...
                        _logger.LogWarning("TryParseExact AdjustToUniversal kullanılmasına rağmen Kind Utc değil: {Kind}", result.Kind);
                ...
                else
                {
                    ...
                    _logger.LogError("DateTime string'i parse edilemedi: '{DateString}'. Beklenen format: '{ExpectedFormat}'.", dateString, ExpectedApiFormat);
                    throw new JsonException($"Unable to parse DateTime string '{dateString}'. Expected format: '{ExpectedApiFormat}'.");
                }
            }
            _logger.LogError("DateTime parse edilirken beklenmedik token tipi: {TokenType}", reader.TokenType);
        ...
        private static readonly ILogger<CustomDateTimeConverter> _logger = LoggerFactory.Create(builder => builder.AddConsole()).CreateLogger<CustomDateTimeConverter>();
