...
namespace FinTrackWebApi.Services.ChatBotService
{
    public class ChatBotService : IChatBotService
    {
        ...
        public async Task<ChatResponseDto> SendMessageToPythonServiceAsync(
        ...
        )
        {
            ...
            try
            {
                ...
                _logger.LogInformation(
                    "Sending a request to the Python ChatBot service: {Url}...",
                    pythonChatBotUrl
                );
                ...
                if (responseFromPython.IsSuccessStatusCode)
                {
                    ...
                    return chatResponse
                        ?? new ChatResponseDto { Reply = "Empty response received from ChatBot." };
                }
                else
                {
                    var errorBody = await responseFromPython.Content.ReadAsStringAsync();
                    _logger.LogError(
                        "Error response from Python ChatBot service. Status: {StatusCode}, Body: {ErrorBody}",
                        responseFromPython.StatusCode,
                        errorBody
                    );
                    return new ChatResponseDto
                    {
                        Reply = "There was a problem communicating with the ChatBot.",
                    };
                }
            }
            catch (Exception ex)
